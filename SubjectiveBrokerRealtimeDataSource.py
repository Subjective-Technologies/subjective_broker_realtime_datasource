...
class SubjectiveBrokerRealtimeDataSource(SubjectiveDataSource):
    connection_type = "BrokerFeed"
    connection_fields = ["broker_name", "api_key", "api_secret", "base_url"]
    ...
    def start(self):
        from com_goldenthinker_trade_datasource.BrokerRealTimeDataSource import StrategyRealTimeDataSource

        name = self._get_param("name", "strategy_realtime")
        ds = StrategyRealTimeDataSource(name=name)
        ds.subscribe(self)
        ds.start()
